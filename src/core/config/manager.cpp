...
void ConfigManager::parse_json_config(const std::string& json_config) {
    ...
    if (root.isMember("dhcp")) {
        ...
        if (dhcp.isMember("security")) {
            ...
            if (security.isMember("policy_file")) {
                config_.security_policy_file = security["policy_file"].asString();
            }
        }

        if (dhcp.isMember("lease_file")) {
            config_.lease_file = dhcp["lease_file"].asString();
        }
        if (dhcp.isMember("server_identifier")) {
            config_.server_identifier = string_to_ip(dhcp["server_identifier"].asString());
        }
        if (dhcp.isMember("advanced_lease_database")) {
            config_.advanced_lease_database = dhcp["advanced_lease_database"].asString();
        }
        if (dhcp.isMember("decline_hold_seconds")) {
            config_.decline_hold_seconds = static_cast<uint32_t>(dhcp["decline_hold_seconds"].asUInt());
        }
        ...
        if (!dhcp.isMember("listen") || !dhcp.isMember("subnets")) {
            throw ConfigException("JSON configuration must include dhcp.listen and dhcp.subnets");
        }
    }
}
...
DhcpSubnet ConfigManager::parse_subnet_config(const std::string& subnet_json) {
    ...
    if (root.isMember("network")) {
        ...
        if (slash_pos != std::string::npos) {
            ...
            subnet.network = string_to_ip(network_str);
            if (subnet.network == 0) {
                throw ConfigException("Invalid network format: " + network_str);
            }
            if (root.isMember("prefix_length")) {
                subnet.prefix_length = static_cast<uint8_t>(root["prefix_length"].asInt());
            } else {
                throw ConfigException("network without CIDR requires prefix_length: " + network_str);
            }
        }
    }

    if (root.isMember("range_start")) {
        subnet.range_start = string_to_ip(root["range_start"].asString());
    }
    if (root.isMember("range_end")) {
        subnet.range_end = string_to_ip(root["range_end"].asString());
    }
...
DhcpConfig get_default_config() {
    ...
    config.server_identifier = 0;
    config.security_policy_file.clear();
    config.advanced_lease_database.clear();
    config.decline_hold_seconds = 3600;
