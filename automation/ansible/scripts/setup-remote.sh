...
useradd -m -s /bin/bash dhcpddev || true
usermod -aG sudo dhcpddev || true

# Create project directory
echo "Setting up project directory..."
PROJECT_DIR="/opt/simple-dhcpd"
mkdir -p $PROJECT_DIR
chown dhcpddev:dhcpddev $PROJECT_DIR

# Create log directories
echo "Creating log directories..."
mkdir -p /var/log/simple-dhcpd
chown dhcpddev:dhcpddev /var/log/simple-dhcpd

# Create config directory
echo "Creating config directory..."
mkdir -p /etc/simple-dhcpd
chown dhcpddev:dhcpddev /etc/simple-dhcpd
...
echo "2. Run: cd $PROJECT_DIR && sudo -u dhcpddev ./build.sh"
echo "3. Test with: sudo -u dhcpddev make test"
