...
#ifndef SIMPLE_DHCPD_UTILS_UTILS_HPP
#define SIMPLE_DHCPD_UTILS_UTILS_HPP
...
/** IPv4 address as four octets (for DHCP options). */
inline std::vector<uint8_t> ip_to_bytes_be(IpAddress ip) {
    uint32_t n = ntohl(ip);
    return {
...
/** Subnet mask in network byte order for CIDR prefix length (IPv4). */
inline IpAddress subnet_mask_for_prefix(uint8_t prefix_length) {
    if (prefix_length >= 32) {
    ...
    return htonl(0xFFFFFFFFu << (32 - prefix_length));
}

/** Big-endian four bytes for a host-order 32-bit value (e.g. lease time in DHCP options). */
inline std::vector<uint8_t> uint32_to_option_bytes(uint32_t host_value) {
    uint32_t n = htonl(host_value);
...
#endif // SIMPLE_DHCPD_UTILS_UTILS_HPP
