#!/bin/bash
# Create DMG installer for simple-dhcpd
...
VERSION="${1:-1.0.0}"
DMG_NAME="$simple-dhcpd-${VERSION}"
DMG_TEMP="dmg-temp"
DMG_FILE="${DMG_NAME}.dmg"
APP_NAME="$simple-dhcpd.app"
PKG_NAME="$simple-dhcpd.pkg"
...
if [ -f "../assets/icons/$simple-dhcpd.icns" ]; then
    # Set custom icon (requires additional tools)
    echo "Custom icon available at ../assets/icons/$simple-dhcpd.icns"
fi
