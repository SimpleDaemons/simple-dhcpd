...
Version:        ${VERSION}
Release:        1%{?dist}
Summary:        simple-dhcpd - A lightweight and secure daemon
License:        Apache-2.0
...
%description
simple-dhcpd is a modern, secure implementation of the DHCP protocol.
...
if [ "$1" -eq 1 ]; then
    ...
    # Create service user
    if ! id dhcpddev &>/dev/null; then
        useradd -r -s /sbin/nologin -d /var/lib/%{name} -c "%{name} service user" dhcpddev
    fi
fi
