...
# simple-dhcpd macOS Build Script
# This script builds the simple-dhcpd application for macOS
...
create_static_package() {
    if [ "$ENABLE_STATIC_LINKING" = "ON" ]; then
        ...
        STATIC_DIR="$DIST_DIR/simple-dhcpd-$VERSION-static-macos"
        mkdir -p "$STATIC_DIR"
        
        # Copy binary and files
        cp "$BUILD_DIR/simple-dhcpd" "$STATIC_DIR/"
        ...
        # Create tarball
        cd "$DIST_DIR"
        tar -czf "simple-dhcpd-$VERSION-static-macos.tar.gz" "simple-dhcpd-$VERSION-static-macos"
        rm -rf "simple-dhcpd-$VERSION-static-macos"
        
        print_success "Static binary package created: simple-dhcpd-$VERSION-static-macos.tar.gz"
    fi
}
