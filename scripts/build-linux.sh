#!/bin/bash
# Generic Linux build script for simple-dhcpd
...
build_project() {
    print_status "Building simple-dhcpd..."
...
install_project() {
    if [ "$INSTALL" = "true" ]; then
        print_status "Installing simple-dhcpd..."
        cd "$BUILD_DIR"
        
        sudo make install
        
        # Test installation
        if command_exists simple-dhcpd; then
...
create_service() {
    if [ "$SERVICE" = "true" ]; then
        ...
        sudo tee /etc/systemd/system/simple-dhcpd.service > /dev/null << EOF
[Unit]
Description=A lightweight and secure DHCP server daemon
...
User=simple-dhcpd
Group=simple-dhcpd
ExecStart=/usr/local/bin/simple-dhcpd --daemon
Restart=always
RestartSec=5
...
EOF
        ...
        sudo useradd --system --no-create-home --shell /bin/false simple-dhcpd 2>/dev/null || true
        ...
        print_status "Use 'sudo systemctl start simple-dhcpd' to start the service"
    fi
}
...
show_usage() {
    cat << EOF
simple-dhcpd - Linux Build Script
...
# Main execution
main() {
    print_status "Starting simple-dhcpd build process..."
    ...
    print_success "simple-dhcpd build process completed successfully!"
}
