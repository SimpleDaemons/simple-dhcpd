...
:build_project
call :print_status "Building simple-dhcpd..."
...
:install_project
if "%INSTALL%"=="true" (
    call :print_status "Installing simple-dhcpd..."
    cd /d "%BUILD_DIR%"
...
if "%ENABLE_STATIC_LINKING%"=="ON" (
    ...
    set "STATIC_DIR=%DIST_DIR%\simple-dhcpd-0.1.0-static-windows"
    if not exist "%STATIC_DIR%" mkdir "%STATIC_DIR%"
    
    REM Copy binary and files
    copy "%BUILD_DIR%\simple-dhcpd.exe" "%STATIC_DIR%\"
    ...
    cd /d "%DIST_DIR%"
    powershell -Command "Compress-Archive -Path 'simple-dhcpd-0.1.0-static-windows' -DestinationPath 'simple-dhcpd-0.1.0-static-windows.zip' -Force"
    rmdir /s /q "simple-dhcpd-0.1.0-static-windows"
    
    call :print_success "Static binary package created: simple-dhcpd-0.1.0-static-windows.zip"
)
goto :eof

REM Function to show usage
:show_usage
echo simple-dhcpd - Windows Build Script
echo.
...
REM Main execution
:main
call :print_status "Starting simple-dhcpd build process..."
...
call :print_success "simple-dhcpd build process completed successfully!"
goto :end
